...
def lambda_handler(event, context):
    ...
    try:
        ...
        if s3_key.startswith('DecryptedFiles/SIG-CWH-TFS/BMCVisits'):
...
def start_glue_job(glue_client, s3_bucket, s3_key):
    response = glue_client.start_job_run(
        JobName='SIG_BMCVisits_GLue_Job',
        Arguments={
...
def wait_for_job_completion(glue_client, job_run_id):
    while True:
        job_status = glue_client.get_job_run(JobName='SIG_BMCVisits_GLue_Job', RunId=job_run_id)
...
def handle_job_result(status, job_status, s3_key, sns_client, sns_topic_arn):
    sns_subject = f'Glue Job Completed - SIG_BMCVisits_GLue_Job - Status: {status}'
    sns_message = f'The Glue job SIG_BMCVisits_GLue_Job has {status} for file {s3_key}.'
    
    if status == 'SUCCEEDED':
        logger.info(f'Glue job SIG_BMCVisits_GLue_Job completed successfully for file {s3_key}.')
    else:
        error_message = job_status['JobRun'].get('ErrorMessage', 'No error message provided')
        logger.error(f'Glue job SIG_BMCVisits_GLue_Job failed for file {s3_key}. Status: {status}. Error: {error_message}')
...
def handle_glue_error(e, sns_client, sns_topic_arn):
    ...
    sns_subject = 'Glue Job Error - SIG_BMCVisits_GLue_Job'
    sns_message = f"""
    Error occurred while processing Glue job SIG_BMCVisits_GLue_Job.
...
def handle_error(e, sns_client, sns_topic_arn, error_type, status_code):
    logger.error(f"{error_type}: {e}")
    sns_subject = 'Lambda Function Error - SIG_BMCVisits_GLue_Job'
    sns_message = f"""
    Error occurred in Lambda function for SIG_BMCVisits_GLue_Job.
    Error type: {error_type}
    Error message: {str(e)}
    """
    publish_sns_message(sns_client, sns_topic_arn, sns_subject, sns_message)
